...
int curr_offset;        /* offset direction for the current trial */
...
#define FPCOLOR_INI		1		/* color before fixation attained */
#define FPCOLOR_AFT		1		/* color after fixation attained */

#define FPDIAM_INI		5		/* diam after fixation attained, prev 3 */
#define FPDIAM_AFT		5		/* diam after fixation attained, prev 4*/
...
printf("autoinit start\n");
	...
	gl_rec = pr_initV(0, 0, 
		...
		"dots_adpt2", 1,	/* fixation task with adaptation to motion switch */
		"ft",		 1,		/* VGS */
		NULL);
...
void rinitf(void)
{
	...
	*/
	if(first_time) {
      ...
      printf("**** DEBUG: rinitf, first_time=%d\n",first_time);
		/* init the screen */
		pr_setup();
	}
	printf("rinitf end");
...
** 
** Switches motion direction between Angle_o and (Angle_o + curr_offset) 
** Keep this function as minimal as possible. 
** 2014/2/17
*/
int do_switch_dir(long ecode)
{
   delta_flag = -delta_flag;
	curr_angle = curr_angle + delta_flag*curr_offset;
	
	//printf("dir=%d\n", curr_angle);
   //printf("dropped ecode: %d\n", ecode);
...
/* ROUTINE: do_t4init
** Adaptation task
**  - get current trial's initial dir (from the sign of coherence in trial list)
**  - get current trial's offset value (from the sign of rate condition)
*/
int do_t4init(void)
{
	long coh_angle = PL_L2PV(gl_rec->trialP->list,1);  /* signed coherence */
	long Angle_o   = TIMV("Angle_o");  // pref angle
	long Angle_d1  = TIMV("Angle_d1"); // offset1
	long Angle_d2  = TIMV("Angle_d2"); // offset2
	long rate_ind  = get_index_rate(); // rate index: -4, -3, -2, -1, 1, 2, 3, 4 
   int  delta_flag_tmp; 
 
   /*send ecodes*/
   ec_send_code_tagged(I_ANGLEO, 7000 + Angle_o);
   ec_send_code_tagged(I_ANGLED1, 7000 + Angle_d1);
   ec_send_code_tagged(I_ANGLED2, 7000 + Angle_d2);
   ...
   if(labs(rate_ind)==1)  /* flip the flag when no switch was in the first half of motion */
		delta_flag = -delta_flag; 

   if(rate_ind>0){
      curr_offset = Angle_d1; 
   }else{
      curr_offset = Angle_d2;
   } 
   
   delta_flag_tmp = delta_flag;
   if(delta_flag_tmp==-1)
		delta_flag_tmp = 0;
	
	curr_angle = Angle_o + delta_flag_tmp * curr_offset;
	
	// the ecode for index_rate is sent in the task c file.
	printf("coh_angle=%d, rate_idx=%d, Angle_o=%d, Angle_d = %d, First dir=%d\n", coh_angle, rate_ind, Angle_o, curr_offset, curr_angle);
	dx_set_by_name(DX_DOTS, DX_DIR, 0, curr_angle);
	return(0);
}

/* Get index for the rate of motion-dir change: 1,slow; 2,mid; 3,fast; 4,very fast
** sign of rate_index indicates which offset will be used.
*/
...
/* take absolute value of rate_index and subtract one. Now the rate index should be
**  0,slow; 1,mid, 2,fast, 3,very fast
*/
int get_index_rate2(void){
   int index_rate = PL_L2PV(gl_rec->trialP->list,0);
   index_rate = abs(index_rate)-1;
/*	printf("change-rate index = %d\n", index_rate); */
	return(index_rate);
}
...
/* TASK 1: passive fixation with dots  */
   ...
   t1start:
	...
	t1wait2:
		...
		to fixbreak on +WD0_XY & eyeflag  /* TD added this line 2014-02-18 */
...
/*		do dx_show_fp(FIX1CD,0,FPDIAM_AFT,FPDIAM_AFT,FPCOLOR_AFT,FPCOLOR_AFT)*/
	...
	t4jmp:
		to t4showd00 on 0 % get_index_rate2
		to t4showd10 on 1 % get_index_rate2
		to t4showd20 on 2 % get_index_rate2
		to t4showd30 on 3 % get_index_rate2
   ...
   /* 10 states in the first half of Dots */
	t4showd30:
		do dx_toggle2(GORANDCD, 1, 0, 0, 3, 1000)
		to t4showd30wait on DX_MSG % dx_check
	t4showd30wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd31
	t4showd31:
		do do_switch_dir(DIRCHG0)
		to t4showd31wait on DX_MSG % dx_check
	t4showd31wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd32
	t4showd32:
		do do_switch_dir(DIRCHG1)
		to t4showd32wait on DX_MSG % dx_check
	t4showd32wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd33
	t4showd33:
		do do_switch_dir(DIRCHG2)
		to t4showd33wait on DX_MSG % dx_check
	t4showd33wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd34
    t4showd34:
		...
		to t4showd34wait on DX_MSG % dx_check
	t4showd34wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd35
	t4showd35:
		do do_switch_dir(DIRCHG4)
		to t4showd35wait on DX_MSG % dx_check
	t4showd35wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd36
	t4showd36:
		do do_switch_dir(DIRCHG5)
		to t4showd36wait on DX_MSG % dx_check
	t4showd36wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd37
	t4showd37:
		do do_switch_dir(DIRCHG6)
		to t4showd37wait on DX_MSG % dx_check
	t4showd37wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd38
	t4showd38:
		do do_switch_dir(DIRCHG7)
		to t4showd38wait on DX_MSG % dx_check
	t4showd38wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showd39
	t4showd39:
		do do_switch_dir(DIRCHG8)
		to t4showd39wait on DX_MSG % dx_check
	t4showd39wait:
		time 100 
		to fixbreak on +WD0_XY & eyeflag
		to t4showdsec1
