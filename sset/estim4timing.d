...
int estimcheck_pre(void)
{
      ...
      if(estim){
      	switch(estim){
			case 1:
	      	startT   = TSMV("onset_e1"); /* Its absolute value must be < minimum possible value for time_random.*/
		      duration = TSMV("duration_e1"); 	
				break;

			case 2:
	      	startT   = TSMV("onset_e2"); /* Its absolute value must be < minimum possible value for time_random.*/
		      duration = TSMV("duration_e2"); 	
				break;

			case 3:
	      	startT   = TSMV("onset_e3"); /* Its absolute value must be < minimum possible value for time_random.*/
		      duration = TSMV("duration_e3"); 	
				break;

			case 4:
	      	startT   = TSMV("onset_e4"); /* Its absolute value must be < minimum possible value for time_random.*/
		      duration = TSMV("duration_e4"); 	
				break;

			default:
				break;
			}
         if(duration<0) duration = 10000; /* 10sec. This effectively turns off estim at sac onset or ncerr.*/
...
status ON
      ...
      estim_on:
            ...
            to estim_wait 
      estim_wait:
            time 500   /* override depending on estim (Taka) */
