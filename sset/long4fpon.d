/*	long4fpon.d
** 	modifed from long4.d. Keep fix pt on when dots appear on dotsrt task. For Ava's use
...
** Long Ding 2015-01-30 when running on the superlogics computer, encountered A/D error after 3-4 correct trials.
** 			this seems to be related to pr_beep_reward routine. Now switch back to the "manual" pulses for reward on/off.
...
int start_dots( void )
{
	int smallrew, bigrew, smallrew_num, bigrew_num;
	int angle0, angle_cur;
	int coh, maxcoh;
   	int p_omit; /* prob of omitting reward in correct trials, in percent */
	int p_estim;
...
/*	set_times("rewarddelay_dots", TTMV("delay2rew"), -1); */
	...
	smallrew = TPMV("smallreward");
	bigrew = TPMV("bigreward");
	smallrew_num = TPMV("SR_Num");
	bigrew_num = TPMV("BR_Num"); 
	flag_asymRew = PL_L2PV(gl_rec->trialP->list, REWINDEX);
	if ( flag_asymRew == 2) {
		/* equal reward */
		currentrew = (smallrew+bigrew)/2;
		...
		if (flag_asymRew) {
			currentrew = smallrew;
			currentrewNum = smallrew_num; 
			if (angle_cur == angle0) {
				currentrew = bigrew;
				currentrewNum = bigrew_num;
			}
		} else {
			currentrew = bigrew;
			currentrewNum = bigrew_num;
...
printf("currentrewNum: %d bigrew_num %d smallrew_num %d \n", currentrewNum, bigrew_num, smallrew_num);
...
/*      if (currentrew == smallrew) { */
   ...
   /* set rtvars */
   pr_set_rtvar("currentrew", currentrew);
   pr_set_rtvar("currentrewNum", currentrewNum);
   pr_set_rtvar("omitrew", flag_omitreward);
...
int fun_rewardon(long code)
{
	...
	if ( currentrewNum>0 && code == REWCD)
		currentrewNum = currentrewNum - 1;
...
*/ /* commented out by Long Ding 2015-01-30 */
	
	totalrew = totalrew + currentrew * currentrewNum; 
  	pr_set_rtvar("totalrew", totalrew);
	set_times("rew_duration", currentrew, -1); 
/* in matlab spm, rew_on/off is detected as DIO input, instead of using the REWCD/REWOFFCD. 
** Currently reward is delivered as rewon1(pulse up)___rew_duration___rewoff1(down)_rewon2(up)_rewoff2(down) 
*/
	pr_trial_set_pmf_reward(gl_rec->trialP, 1);
	return(0);
...
*/ /* revert to manual pulses for reward bit, Long Ding 2015-01-30 */
...
*/ /* commented out by Long Ding 2015-01-30, replace with manual pulse */
