...
long estim         = 0;     // 0 for no estim, 1 for estim. Specify corresponding to "estim" in the task_dotRT_stim_RevCorr.c
...
int start_dots( void )
{
	...
	/* set basic task timing */
      ...
      set_times("estim_wait",      TSMV("duration"),   -1);
	...
	*/
	...
	estim = PL_L2PV(gl_rec->trialP->list, ESTIMINDEX); // estim: 0, no stim; 1, pulse stim; 2, standard stim
...
/**************************************************
******* Random pulses of microstimulation *********
**************************************************/
estim_set {
status ON
      begin estim_rest:
      ...
      estim_initdelay:
            do set_estim_delay_timer()              /* Counters for ecodes are reset to zero here. */
            to estim_rand on MET % timer_check3
            to estim_rest on   0 = estim_flag
      estim_rand:
            to estim_on on MET % etim_on_or_off
            to estim_off
      estim_on:
      ...
      estim_off:
            ...
            to estim_wait
      estim_wait:
            time 200                        /* override by set_times() (Taka) */
