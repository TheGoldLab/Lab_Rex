...
int do_t4init(void)
{
	/* for now, assume the change_val is post change */
	/* grab the value */
	int changek;
	
	printf("do_t4init start\n");
	changek = PL_L2PV(gl_rec->trialP->list,1);  /* get the index from the trial list */
	switch(changek){
		case 0:
			printf("case 0\n");
			change_val = TIMV("Change_val0");
			break;
		case 1:
			printf("case 1\n");
			change_val = TIMV("Change_val1");
			break;
		case 2:
			printf("case 2\n");
			change_val = TIMV("Change_val2");
			break;
		case 3:
			printf("case 3\n");
			change_val = TIMV("Change_val3");
			break;
	}
	
	printf("changek=%d, change_val=%d\n", changek, change_val);
	ec_send_code_tagged(I_COHCD,7000 + change_val);
	/* pl_ecodes_by_nameV(dx_get_object(DX_DOTS), 7000,
	DX_COH,		0, 	I_COHCD,
	NULL); */
	
	printf("do_t4init end\n");
	return(0);
}
...
/*		do dx_show_fp(FIX1CD,0,FPDIAM_AFT,FPDIAM_AFT,FPCOLOR_AFT,FPCOLOR_AFT)*/
	...
	t4showT:
		do do_showT()
/*		do dx_toggle2(TARGONCD, 1, 1, 1000, 2, 1000) */
		to t4winpos1 on DX_MSG % dx_check
	t4winpos1:
		do dx_position_window(TGWINSIZE, TGWINSIZE, 1, 0, 1)
		time 100
	    to t4winpos2
	...
	t4wait2:
		do timer_set1(1000, 300, 800, 500, 500, 0)
		to fixbreak on +WD0_XY & eyeflag
 		to t4fpdim on MET % timer_check1
	
	/* Show the "dimmed" fp */
	t4fpdim:
		do do_show_fp2()
		to t4showd on DX_MSG % dx_check
	/* Show the dots */
	t4showd:
		do dx_toggle2(GORANDCD, 1, 0, 0, 3, 1000)
		to t4waitpre on DX_MSG % dx_check
	t4waitpre:  /* wait before stimulus change */
		do timer_set1(0,0,0,0,VSTIM_DUR,0)  
		to fixbreak_detect on +WD0_XY & eyeflag
		to t4change on MET % timer_check1
	t4change:
		do do_set_change()
		to t4waitpost1 on DX_MSG % dx_check 
	t4waitpost1:  /* response in this window is still a false-alarm */
		time 200
		do show_dummy_wind(1)
		to fixbreak_detect on +WD0_XY & eyeflag
		to t4waitpost2
	t4waitpost2:   /* this is how much has before animal must saccade */
		do timer_set1(0,0,0,0,1000,0)
		to t4sacon on +WD0_XY & eyeflag	/* if "fixbreak" monkey did right thing!*/    
		to ncerr   on MET % timer_check1  /* else, it's a no-choice error */
	t4sacon:
		do dx_toggle2(SACMAD, 0, 0, 1000, 3, 1000) /*TD 2013-12-10, not sure if args are correct*/
		to t4wait4hit on DX_MSG % dx_check
	t4wait4hit:
		time 300   /*TD, a short grace period to allow only a valid saccade*/
...
#-11:16777215
#0:18162:default:-3:-3:0
#18162:18166:monospace10:-3:-3:0
#18166:19107:default:-3:-3:0
#19107:19111:monospace10:0:-1:0
#19111:23363:default:-3:-3:0
