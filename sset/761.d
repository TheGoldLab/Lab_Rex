   ...
   */
	...
	*/
	...
	fpset:
		...
		to fpwin2
	fpwin2:
		time 350 /* again time to settle window */
...
/* TASK 1: RG dots  */
	...
	t1show1:
		do dx_toggle2(PSEUDTRGCD, 1, 1, 1000, 2, 1000)
		to t1wait2 on DX_MSG % dx_check

	/* Second wait period before changing the targets */
	t1wait2:
	   do timer_set1(0, 0, 0, 0, 500, 0)
	   to fixbreak on +WD0_XY & eyeflag
	   to t1changet1 on MET % timer_check1 
	
	/* Change target color & diam if needed */	
	t1changet1:
		do dx_change_target(1, 3, 8, 2, 2, 8)
		to t1wait3 on DX_MSG % dx_check
	
	/* Third wait period before showing the dots */
	t1wait3:
		do timer_set1(1000, 300, 500, 300, 500, 0)
		to fixbreak on +WD0_XY & eyeflag
	...
	t1showd:
		...
		to t1waitd1 on DX_MSG % dx_check
	
	t1waitd1:
		do timer_set1(0, 0, 0, 0, 300, 0)
		to fixbreak on +WD0_XY & eyeflag
		to t1changet2 on MET % timer_check1
	
	/* Change target color & diam if needed */	
	t1changet2:
		do dx_change_target(1, 3, 8, 2, 2, 8)
		to t1waitd2 on DX_MSG % dx_check
	...
	t1wait4:
		do timer_set1(0, 0, 0, 0, 600, 0)
		to fixbreak on +WD0_XY & eyeflag
 		to t1changet3 on MET % timer_check1
	
	/* Change target color & diam if needed */	
	t1changet3:
		do dx_change_target(1, 3, 8, 2, 2, 8)
		to t1wait5 on DX_MSG % dx_check
...
/* TASK 2: RG dots  */
	...
	t2wait2:
	   do timer_set1(0, 0, 0, 0, 500, 0)
	   to fixbreak on +WD0_XY & eyeflag
	   to t2changet1 on MET % timer_check1 
	...
	t2showd:
		do dx_toggle2(GORANDCD, 1, 0, 0, 3, 1000)
		to t2waitd1 on DX_MSG % dx_check
	
	t2waitd1:
		do timer_set1(0, 0, 0, 0, 300, 0)
		to fixbreak on +WD0_XY & eyeflag
		to t2changet2 on MET % timer_check1
	
	/* Change target color & diam if needed */	
	t2changet2:
		do dx_change_target(1, 3, 8, 2, 2, 8)
		to t2waitd2 on DX_MSG % dx_check
	...
	t2wait4:
		do timer_set1(0, 0, 0, 0, 600, 0)
		to fixbreak on +WD0_XY & eyeflag
 		to t2changet3 on MET % timer_check1
	
	/* Change target color & diam if needed */	
	t2changet3:
		do dx_change_target(1, 3, 8, 2, 2, 8)
		to t2wait5 on DX_MSG % dx_check
