/*	long2_FD.d
** 6-5-09 changed rew_by_RT: slope is constant for all coh, but maxRew is larger for lower coherence
** fixed viewing time for the FD task; to be used for Falco during training
** for Falco: encourage shorter RT for coherence>25.6% right choice, and longer RT for lower coherence
**
   ...
   set_times("fpshow_delay", 1, -1);
	if (task_index>0)
	{
      printf(" flag_asymRew %d  REWINDEX %d \n", flag_asymRew, PL_L2PV(gl_rec->trialP->list, REWINDEX) );
		if ( flag_asymRew!=PL_L2PV(gl_rec->trialP->list, REWINDEX) ) 
		{
...
/* ----------- this section is for training Falco ---------------- */	
	choice = PL_L2PV(gl_rec->trialP->list, ANGLEINDEX);
	coh = abs(choice);
/*	if (choice<0)
		flag_asymRew = 0;
*/
	maxRew = maxRew + (9-log(coh)/log(2))*slopemaxRew;
/*	if  (coh > 256) 
	{
		slope1 = -slope1;
		slope2 = -slope2;
...
int set_viewtimer( long minRT, long maxRT)
{
	int RT;
	int coh, mincoh, maxcoh;
	mincoh = TPMV("Coherence_lo");
	maxcoh = TPMV("Coherence_hi");
	coh = PL_L2PV(gl_rec->trialP->list, 1);	
	coh = abs(coh);
	if (minRT+maxRT == 0) 
	{
		RT = 1000;	
	}
	else if (mincoh == maxcoh)  
	{
		RT = (int)(minRT+maxRT)/2;	
	}
	else if (coh == 0)
	{
		RT = maxRT;
	...
	else	
	{
		if (mincoh == 0)
			mincoh = 1;	
		RT = maxRT - log(coh/mincoh) * (maxRT - minRT)/log(maxcoh/mincoh);
	}
printf("current coh = %d viewing %d\n", coh, RT);
	timer_set1(0, 0, 0, 0, RT, 0);
	return(0);
}
...
//		to fixbreak on +WD0_XY & eyeflag
	...
	rewarddelay_dots:
		do rew_by_RT(20, 800, 20, 900, 500, 1100)	
	...
	finish_dots:
      do pr_finish_trial()
      rl 0
      to wait2nexttrial on MET % timer_check2
	...
	fixoff_reg:
		...
		rl 40
		to setgotime_dotsreg on DX_MSG % dx_check
	setgotime_dotsreg:
		do tgton_time()
		to waitforsac_dotsreg
	waitforsac_dotsreg:
		do dummyfpcueoff()	
