/* 737_Stim.d
**
** Modified from 737.d to include electrical microstimulation
**
** Josh/Sidd 041813 - 051413
**
**
** Old header info for 737.d is below:
**
** 737.d
**
** Countermanding paradigm for Rishi
**
**	created by Rishi 12/11/09
*/
...
int tone_off(void)
{
    ...
    ec_send_code(	TONEOFF); 
return(0); 
	}

/* ROUTINE: Conditional beepulation */
int my_conditional_beep(long id, long prob, long repeat)
{
	static int repeat_index = 0;

	/* only stim is random draw between 0 & 999 < prob */
	if((repeat>0 && repeat_index>0) || (TOY_RAND(1000) < prob)) {
...
/* ROUTINE: Conditional microstimulation */
int my_conditional_stim(long prob, long repeat)
{
	static int repeat_index = 0;
...
begin	first:
   ...
   */
	...
	*/	
	...
	/* Task 2 is ft ... just like task 1, except a tone is presented
	*/
	...
	t2wait2:
		do timer_set1(1000, 100, 600, 200, 0, 0)
		to fixbreak on +WD0_XY & eyeflag
		to tone_1 on MET % timer_check1
   tone_1: 
   		time 3000
/*
   		do dx_beep(BEEPON)
*/
   		do my_conditional_beep(BEEPON,500,2)
   		to fixbreak on +WD0_XY & eyeflag
   		to t2ftc2
/*
   		to tone_0 on +WD0_XY & eyeflag
*/
   	tone_0: 
 			to t2ftc2
/* UNUSED STATE
...
*/
	...
	*/
	...
	t3wait1:
     do timer_set1(1000, 100, 600, 200, 0, 0)
     to fixbreak on +WD0_XY & eyeflag
	  to t3stim on MET % timer_check1
	t3stim:
	  do my_conditional_stim(999, 5, 1000, 0)
	  to t3wait2
	t3wait2:
		do timer_set1(1000, 100, 600, 200, 0, 0)
		to fixbreak on +WD0_XY & eyeflag
...
**   echk:
**      to efailwait on +WD0_XY & eyeflag
**		to etest on 0 % dx_check_fix
**   efailwait:
**   		do timer_set5(0, 0, 0, 0, 2000, 0) 
**		to efail 
**	efail:
