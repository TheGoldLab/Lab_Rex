/* 785yltd2.d
   2013/11/18 td and yl created this paradigm from 785yltd.d for a coherence-change
   detection task.
   As an initial test, the state t1changecoh was added to the fixation task. 
*
*  2013/11/19 yl: added t1postchwait  ... for time to wait after coh change
*/
...
int do_setCoh(long ecode, long coherence)
{
	dx_set_ecode(ecode);
	dx_set_by_name(DX_DOTS, DX_COH, 0, coherence);	
...
/*	t1waitd:
	...
	t1waitd: 
		do timer_set1(0,0,0,0,VSTIM_MAP_DUR,0)
		to fixbreak on +WD0_XY & eyeflag
		to t1changecoh on MET % timer_check1
   t1changecoh:
      time 500  /* this doesn't seem to do anything */
      do do_setCoh(COHCHG,50) 
      to t1postchwait on DX_MSG % dx_check
    t1postchwait:
...
/*		do dx_position_window(50, 50, 2, 0, 2) */
	...
	t3showd:
		do dx_toggle2(GORANDCD, 1, 0, 0, 3, 1000)
		to t3waitpre on DX_MSG % dx_check
/*		to t3trystim_on on DX_MSG % dx_check
	t3trystim_on:
		do try_toggle_MT_stim(1)
		to t3waitpre */
	t3waitpre:  /* wait before coherent stimulus comes on */
		do timer_set1(0,0,0,0,VSTIM_DUR,0)  
		to t3changecoh on MET % timer_check1
	t3changecoh:
		do do_setCoh(COHCHG,500)
		to t3waitd on DX_MSG % dx_check 
	t3waitd:   /* this is how much has before animal must saccade */
		do timer_set1(0,0,0,0,1000,0)
		to grace on +WD0_XY & eyeflag	/* if "fixbreak" monkey did right thing!*/    
...
#-11:16777215
#0:18242:default:-3:-3:0
#18242:18246:monospace10:-3:-3:0
#18246:22575:default:-3:-3:0
