...
/*
 ...
 2014-04-29. TD. Removed 3.2% from coherence set.
 
 */

/*
 task_dotvar_TD.c (variable duration)
 Created by Taka Doi from task_dotreg_TD.c on 2015-07-14
 to run a variable duration Dots task and see if Anubis is doing the temporal integration
 */
...
/* PRIVATE ROUTINES */
void 		dotsvarTD_make_trials	(_PRtask);
_PRtrial dotsvarTD_get_trial	(_PRtask, int);
void 		dotsvarTD_set_trial	(_PRtrial);
...
struct _TAtask_struct gl_dotsvarTD_task[] = {
	...
	**	Task methods
	*/
	&dotsvarTD_make_trials,
	&dotsvarTD_get_trial,
	&dotsvarTD_set_trial,
};


/* PUBLIC ROUTINE: dotsvarTD_make_trials
**
*/
void dotsvarTD_make_trials(_PRtask task)
{   // 1 is later changed to 0. Use 1 to show % correct correctly in console window. TD
	...
	int  	num_coh = sizeof(coh_list)/sizeof(int),
					 *cohs=NULL, coh_n,
					 coh_lo, coh_hi;
	int i,  k,  *cohs_angle=NULL;
   int cohs_angle_n;
	 
   static int dur_list[] = {175, 350, 525, 700};
   int        num_dur = sizeof(dur_list)/sizeof(int);
   int        *dur=NULL;
	...
	*/
	tu_make_trials2D(task, "dur", num_dur,      dur,        1.0,
...
/* PUBLIC ROUTINE: dotsvarTD_get_trial
**
** Returns:
**		_PRtrial if trials remain
**		NULL otherwise
*/
_PRtrial dotsvarTD_get_trial(_PRtask task, int reset_flag)	
{
    /*
     ...
     **
     ** note that my function starts with td_ not tu_
     */
    
    // 2015-07-15. td_get_random_altRows2() is used to make a trial block with randomized cohs and randomized durations
    return(td_get_random_altRows2(task, reset_flag));
}



/* PUBLIC ROUTINE: dotsvarTD_set_trial
**
**	Note that here we mostly access groups/lists/properties
**	 by INDEX, not NAME, so be careful if anything
**	 above changes.
*/
void dotsvarTD_set_trial(_PRtrial trial)
{
	/*
