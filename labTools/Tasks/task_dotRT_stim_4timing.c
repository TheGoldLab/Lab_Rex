...
/* PRIVATE ROUTINES */
void        estim_4t_make_trials(_PRtask);
_PRtrial    estim_4t_get_trial(_PRtask, int);
void        estim_4t_set_trial(_PRtrial);

/* 
** TASK DOTSRT
**		("estim_4t") ,meaning 4 timing
*/

struct _TAtask_struct gl_estim_4t_task[] = {
   ...
   ** Menus 
	...
	*/
	{ "Basic-Property",		 1 },
            { "Trial_order",		 0, 1.0 },
            { "Trial_blocks_e0",	 0, 1.0 },	/* # of blocks for no stim */
            { "Trial_blocks_e1",	 0, 1.0 },	/* # of blocks for full stim */
            { "Trial_blocks_e2",	 0, 1.0 },	/* # of blocks for partial stim 1 */
            { "Trial_blocks_e3",	 0, 1.0 },	/* # of blocks for partial stim 2 */
            { "Trial_blocks_e4",	 0, 1.0 },	/* # of blocks for partial stim 3 */
            { "Angle_o",      	 0, 1.0 },
            { "Coherence_lo", 	 0, 1.0 },
            { "Coherence_hi", 	 0, 1.0 },
      ...
      */                             
      { "Estim-Property",     1 },
            { "onset_e1",       0, 1.0 },    // onset time relative to Dots on 
            { "duration_e1",    0, 1.0 },    // duration (-1 menas microstim until sac) 
            { "onset_e2",       0, 1.0 },    // onset time relative to Dots on 
            { "duration_e2",    0, 1.0 },    // duration (-1 menas microstim until sac) 
            { "onset_e3",       0, 1.0 },    // onset time relative to Dots on 
            { "duration_e3",    0, 1.0 },    // duration (-1 menas microstim until sac) 
            { "onset_e4",       0, 1.0 },    // onset time relative to Dots on 
            { "duration_e4",    0, 1.0 },    // duration (-1 menas microstim until sac) 
            {NULL},
      /////////////////////////////////////////////////////////////////////
...
**
*/
void estim_4t_make_trials(_PRtask task)
{
	...
	*/
	...
	for (i=0; i<coh_n; i++) {
      ...
      ///////////////////////////////////////////////////////////////////////////////////////////////	
      ///////////////////////////////////////////////////////////////////////////////////////////////
   estim_n = 5;  // 0, no stim; 1, full stim; 2, early; 3, middle or late; 4, end.  
...

/* PUBLIC ROUTINE: dotsRTestim4t_get_random_byCoh
**
...
_PRtrial dotsRTestim4t_get_random_byCoh(_PRtask task, int num_blocks[], int reset_flag, int num_by_coh, int num_zero_coh)
{
   ...
   if(reset_flag && !task->trialPs){
      ...
      int total_num_coh[] = {0,0,0,0,0}; // [0] for no stim, [1] for full stim, [2] for early stim, [3] for late stim, [4] for end stim.
      ...
      */ 
      ...
      printf("#block mid (or late) estim,     %d\n", numTrialStim[3]);
      printf("#block end   estim,     %d\n", numTrialStim[4]);
      //-------------------------------------------------------------------
      ...
      */
      ...
      printf("num_blocks estim off %d, num_blocks full estim %d, num_blocks early estim %d, num_blocks late estim %d, num_blocks end estim %d\n", numTrialStim[0], numTrialStim[1], numTrialStim[2], numTrialStim[3], numTrialStim[4]); 
...
**		NULL otherwise
*/
_PRtrial estim_4t_get_trial(_PRtask task, int reset_flag)	
{
   ...
   int trial_blocks[] = {0,0,0,0,0};
	...
	trial_blocks[0] = TPMV(task, "Trial_blocks_e0");
   ...
   num_zero_coh = TPMV(task, "num_zero_coh");
	...
	return(dotsRTestim4t_get_random_byCoh(task, trial_blocks, reset_flag, num_by_coh, num_zero_coh));
}
...
void estim_4t_set_trial(_PRtrial trial)
{
   ...
   printf("dotRT_estim4t trial  estim = %d angle = %d coh = %d Ecc = %d\n", 
  ...
  // printf("estim_4t_set_trial done \n");
}
