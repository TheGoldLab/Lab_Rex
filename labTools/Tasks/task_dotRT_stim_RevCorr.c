/*
  ...
  -- History -- 
       ...
       2012-04-02, Extend the task so that the number of blocks can be chosen for each estim type. Specifically,
                   I want to exclude standard stim condition sometimes or pulse condition other times.i
                    - "percent_stim" menu will be omitted,   
*/
...
/* PRIVATE ROUTINES */
void        estim_rc_make_trials(_PRtask);
_PRtrial    estim_rc_get_trial(_PRtask, int);
void        estim_rc_set_trial(_PRtrial);
...
struct _TAtask_struct gl_estim_rc_task[] = {
   ...
   ** Menus 
	...
	*/
	{ "Basic-Property",		 1 },
            { "Trial_order",		 0, 1.0 },
            { "Trial_blocks_e0",	 0, 1.0 },	/* Trial_blocks for no estim */
            { "Trial_blocks_e1",	 0, 1.0 },	/* for pulse stim */
            { "Trial_blocks_e2",	 0, 1.0 },	/* for full stim */
            { "Angle_o",      	 0, 1.0 },
            { "Coherence_lo", 	 0, 1.0 },
            { "Coherence_hi", 	 0, 1.0 },
...
**
*/
void estim_rc_make_trials(_PRtask task)
{
	...
	*/
      ///////////////////////////////////////////////////////////////////////////////////////////////
      estim_n = 3;  // 0, no stim; 1, pulse stim; 2, standard stim. 
...

/* PUBLIC ROUTINE: dotsRTestimRC_get_random_byCoh
**
...
_PRtrial dotsRTestimRC_get_random_byCoh(_PRtask task, int num_blocks[], int reset_flag, int num_by_coh, int num_zero_coh)
{
   ...
   if(reset_flag && !task->trialPs){
      ...
      int total_num_coh[] = {0,0,0}; // [0] for no stim, [1] for pulse stim, [2] for standard stim.
      ...
      */ 
      ...
      numTrialStim[0] = num_blocks[0];	// estim off
      numTrialStim[1] = num_blocks[1];	// estim pulse 
      numTrialStim[2] = num_blocks[2];	// estim full 

      // n = numTrialStim[1]+numTrialStim[2];   // probably unnecesary?   // # of blocks for estim on.
      
   	num_blocks_new = num_blocks[0]+num_blocks[1]+num_blocks[2]; // I keep "num_blocks_new" becasue subsequent codes use it. (Taka 09/22/11)
      printf("#block estim off,       %d\n", numTrialStim[0]);
      printf("#block pulse estim,     %d\n", numTrialStim[1]);
      printf("#block standard estim,  %d\n", numTrialStim[2]);
      //-------------------------------------------------------------------
      ...
      */
      ...
      printf("num_blocks estim off %d, num_blocks pulse estim %d, num_blocks std estim %d\n", numTrialStim[0], numTrialStim[1], numTrialStim[2]); 
...
**		NULL otherwise
*/
_PRtrial estim_rc_get_trial(_PRtask task, int reset_flag)	
{
   int num_by_coh;
   int num_zero_coh;
   int trial_blocks[] = {0,0,0};
   ...
   num_zero_coh     = TPMV(task, "num_zero_coh");
	...
	return(dotsRTestimRC_get_random_byCoh(task, trial_blocks, reset_flag, num_by_coh, num_zero_coh));
}
...
void estim_rc_set_trial(_PRtrial trial)
{
   ...
   printf("dotRT_estimRC trial  estim = %d angle = %d coh = %d Ecc = %d\n", 
  ...
  // printf("estim_rc_set_trial done \n");
}
