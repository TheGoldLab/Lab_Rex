...
/* PRIVATE ROUTINES */
void 		dotsFD_make_trials	(_PRtask);
_PRtrial dotsFD_get_trial		(_PRtask, int);
void 		dotsFD_set_trial		(_PRtrial);
...
*/
struct _TAtask_struct gl_dotsFD_task[] = {
...
/* PUBLIC ROUTINE: dotsFD_make_trials
**
*/
void dotsFD_make_trials(_PRtask task)
{
...
/* PUBLIC ROUTINE: dotsFD_get_trial
**
** Returns:
**		_PRtrial if trials remain
**		NULL otherwise
*/
_PRtrial dotsFD_get_trial(_PRtask task, int reset_flag)
{
...
void dotsFD_set_trial(_PRtrial trial)
{
	...
	speed = PL_L2PVS(tdm, 1, 0);

	/* Print stuff for current trial */
	printf("RG Dots trial angle = %d coh = %d\n", ang2use, coh2use);

	/* Fill the values for object 3 & 4 */
	PL_CSET( PL_L2PWS(tgm, 5, 3), ang2use);
	PL_CSET( PL_L2PWS(tgm, 5, 4), ang2use+180);	
	...
	printf("computed xyrt, tx: %d ty:%d\n", PL_L2PWS(tgm, 0, 3), PL_L2PWS(tgm, 1, 3));
	printf("computed xyrt, tlen: %d tlen+1:%d\n", PL_L2PWS(tgm, TU_XYRT_LEN, 3), PL_L2PWS(tgm, TU_XYRT_LEN+1, 3));
