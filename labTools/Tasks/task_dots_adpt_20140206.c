...
/* PRIVATE ROUTINES */
void 		dots_adpt_make_trials	(_PRtask);
_PRtrial dots_adpt_get_trials		(_PRtask, int);
void 		dots_adpt_set_trials		(_PRtrial);
...
*/
struct _TAtask_struct gl_dots_adpt_task[] = {
	...
	**	Task methods
	*/
	&dots_adpt_make_trials,
	&dots_adpt_get_trials,
	&dots_adpt_set_trials,
};

/* PUBLIC ROUTINE: dots_adpt_make_trials
**
*/
void dots_adpt_make_trials(_PRtask task)
{
...
/* PUBLIC ROUTINE: dots_adpt_get_trials
**
** Returns:
**		_PRtrial if trials remain
**		NULL otherwise
*/
_PRtrial dots_adpt_get_trials(_PRtask task, int reset_flag)
{
	int trial_blocks = TIMV(task, "Trial_blocks");  
	switch(TIMV(task, "Trial_order")) {

   printf("DEBUG: dots_adpt_get_trials, check 0");
		/* BLOCK */
		case 0:
...
/* PUBLIC ROUTINE: dots_adpt_set_trials
**
...
*/
void dots_adpt_set_trials(_PRtrial trial)
{
   ...
   int         angle = TIMV(trial->task, "Angle_o"), ang, cohv;
   double		ddir;
    
   printf("DEBUG: dots_adpt_set_trials, check 0\n");
