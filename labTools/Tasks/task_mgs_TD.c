...
** Modified to creat task_mgs_TD.c, 2013-06-03, Taka Doi 
   time out menu variables are created for ncerr and brfix trials.
   mgs is changed to mgsTD
** 2013-06-05 
   New menu variables are created for the exponential timer of delay period.
...
/* PRIVATE ROUTINES */
void 		mgsTD_make_trials	(_PRtask);
_PRtrial mgsTD_get_trial	(_PRtask, int);
void 		mgsTD_set_trial	(_PRtrial);
...
struct _TAtask_struct gl_mgsTD_task[] = {
...
/* PUBLIC ROUTINE: mgsTD_make_trials
**
*/
void mgsTD_make_trials(_PRtask task)
{
...
/* PUBLIC ROUTINE: mgsTD_get_trial
**
** Returns:
**		_PRtrial if trials remain
**		NULL otherwise
*/
_PRtrial mgsTD_get_trial(_PRtask task, int reset_flag)	
{
...
void mgsTD_set_trial(_PRtrial trial)
{
   ...
   // printf("DEBUG: actual X = %d, Y = %d", PL_L2PWS(tgm, 0, 1),PL_L2PWS(tgm, 1, 1));
