...
struct _TAtask_struct gl_explorer_task[] = {
	...
	*/
	...
	/* Graphics objects: the foraging targets, the fixation, etc. */
...
void explorer_make_trials(_PRtask task) 
{
   int blocks[NUM_BLOCK_MENUS] = {0,1,2,3,4,5};
	...
	/*  get info from task menu to set up user data (array of target angles) */
	ud 					= SAFE_STALLOC(_exploreruser_data_struct);
	ud->num_targets 	= TIMV(task,"num_targets");
	ud->angles 	  	= toy_seriesi(((ud->num_targets)-2),TIMV(task,"target_i"),360/((ud->num_targets)-2));
	ud->xs 			  	= NULL;
	ud->ys 			  	= NULL;	
	task->user_data 	= ud;
	...
	** NOTE: ASSUMES USED BLOCKS ARE LISTED FIRST */
	...
	tu_make_trials2D(task,"angle",((ud->num_targets)-2),ud->angles,0.1,"block",i,blocks,1.0);
...
{ 
	...
	*/
	if(PL_L2PV(dxTarget,1) != PL_L2PW(dxTarget,1) ||
		...
		ud->xs   = SAFE_ZALLOC(double,((ud->num_targets)-2));
		ud->ys   = SAFE_ZALLOC(double,((ud->num_targets)-2));
		
		/* set the xs and ys from the given angles; all else from preset
		values or from the task info menu var */
		for(i=0;i<ud->num_targets;i++) {
			if (i==0) {
				/* set the f.p. */
				dx_set_by_name(DX_TARGET,DX_X,i,0);
				dx_set_by_name(DX_TARGET,DX_Y,i,0);
				dx_set_by_name(DX_TARGET,DX_CLUT,i,fp_clut);
				dx_set_by_name(DX_TARGET,DX_DIAM,i,fp_size);
			} else if (i==1) {
				/* set the choice feedback */
				dx_set_by_name(DX_TARGET,DX_X,i,0);
				dx_set_by_name(DX_TARGET,DX_Y,i,0);
				dx_set_by_name(DX_TARGET,DX_CLUT,i,choice_clut);
				dx_set_by_name(DX_TARGET,DX_DIAM,i,choice_diam);
			} else {
				/* update the angles, which aren't setting properly */
				ud->angles[i-2]=(i-2)*(360/((ud->num_targets)-2));
				/* update the properties for the targets */
				dx_set_by_name(DX_TARGET,DX_X,i,TOY_RT_TO_XF(0, target_distance/10., ud->angles[i-2]));
				dx_set_by_name(DX_TARGET,DX_Y,i,TOY_RT_TO_YF(0, target_distance/10., ud->angles[i-2]));
				dx_set_by_name(DX_TARGET,DX_CLUT,i,((double) TIMV(task, "target_clut")));
				dx_set_by_name(DX_TARGET,DX_DIAM,i,((double) TIMV(task, "target_diam"))/10.);
				/* Store the xs and ys in ud */
				ud->xs[i-2] = TOY_RT_TO_XF(0, target_distance/10., ud->angles[i-2]);
				ud->ys[i-2] = TOY_RT_TO_YF(0, target_distance/10., ud->angles[i-2]);
			}
		}
