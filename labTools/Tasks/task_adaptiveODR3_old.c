...
void adODR3_make_trials(_PRtask task)
{
  int i;
   ...
   /* add the array of 2 (empty) trials */
...
_PRtrial adODR3_get_trial(_PRtask task, int reset_flag)
{
	...
	*/
	...
	if(hazard>0 && TOY_RCMP(hazard)) {
			printf(" Switching Target MinorChange\n");
...
		printf(" RepFlag %d /n", pl_list_get_w(gl_rec->paradigm_menu, "Trial_repeats"));
	pl_list_set_v2w(tgm);
...
#-11:16777215
#0:6809:default:-3:-3:0
#6809:6818:TextFont9:-3:-3:0
#6818:8198:default:-3:-3:0
