...
/* PRIVATE ROUTINES */
void 		vgs_make_trials	(_PRtask);
_PRtrial vgs_get_trial	(_PRtask, int);
void 		vgs_set_trial	(_PRtrial);
...
struct _TAtask_struct gl_vgs_task[] = {
...
/* PUBLIC ROUTINE: vgs_make_trials
**
*/
void vgs_make_trials(_PRtask task)
{
...
/* PUBLIC ROUTINE: vgs_get_trial
**
** Returns:
**		_PRtrial if trials remain
**		NULL otherwise
*/
_PRtrial vgs_get_trial(_PRtask task, int reset_flag)	
{
...
void vgs_set_trial(_PRtrial trial)
{
	...
	**	TU_XYRT_LEN refers to the DIAM property
	**	TU_XYRT_LEN+1 refers to the CLUT property
	*/	
