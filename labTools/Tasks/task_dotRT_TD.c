...
/*
 task_dotRT_TD.c
 Created by Taka Doi from task_dotRT_LD20070201.c on 2013-05-30
 for perceptual learning and speed-accuracy tradeoff in FEF-CD
 
 2013-06-06
 How to construct a trial block is greatly modified.
 - A menu variable nTrialpBlock specifies HALF the number of trials per block, on which randomization
 - is done.  
 - The number of trials for each absolute coherence level is determined according to a pdf,p. 
 - p = w*p_highcoh  + (1-w)*p_bias
 - p_bias = y*p_uni + (1-y)*p_beta
 - p_highcoh, highest coherence level only.
 - p_uni    , uniform pdf
 - p_beta   , beta distribution with parameters of alpha and beta
 - w, y, alpha, and beta are menu variables.
...
/* PRIVATE ROUTINES */
void 		dotsrtTD_make_trials	(_PRtask);
_PRtrial dotsrtTD_get_trial	(_PRtask, int);
void 		dotsrtTD_set_trial	(_PRtrial);
...
struct _TAtask_struct gl_dotsrtTD_task[] = {
	...
	*/
	{ "dotsrtTD-Timing",		1 },
        ...
        { "MinFdbkDelay",   	0, 1.0 },
   		...
   		{ "brfixTimeOut",   	0, 1.0 },
   		{ "ncerrTimeOut",   	0, 1.0 },
        { "flagExpTimeOut",   	0, 1.0 },
        { "maxExpTimeOut",   	0, 1.0 },
        { "minExpTimeOut",   	0, 1.0 },
        { "scaleExpTimeOut",   	0, 1.0 },
        { "decayExpTimeOut",   	0, 1.0 },
		{NULL},
	/* 
	...
	**	Task methods
	*/
	&dotsrtTD_make_trials,
	&dotsrtTD_get_trial,
	&dotsrtTD_set_trial,
};


/* PUBLIC ROUTINE: dotsrtTD_make_trials
**
*/
void dotsrtTD_make_trials(_PRtask task)
{
  	static int coh_list[] = {1, 32, 64, 128, 256, 512, 999}; // 1 is later changed to 0. Use 1 to show % correct correctly in console window. TD
	int  	num_coh = sizeof(coh_list)/sizeof(int),
					 *cohs=NULL, *rewcont, coh_n,
					 coh_lo, coh_hi, rewcont_n;
	int i,  k,  *cohs_angle=NULL, cohs_angle_n;
   
//   printf("DEBUG: dotsrtTD_make_trials is entered\n");
	...
	*/
	// cohs_angle = {0,0,32,-32,64,-64,128,-128,....,999,-999}
...
//   printf("DEBUG: dotsrtTD_make_trials is finishing.\n"); 
}

/* PUBLIC ROUTINE: dotsrtTD_get_trial
**
** Returns:
**		_PRtrial if trials remain
**		NULL otherwise
*/
_PRtrial dotsrtTD_get_trial(_PRtask task, int reset_flag)
{   
//    printf("DEBUG: dotsrtTD_get_trial is entered.\n");
    /*
