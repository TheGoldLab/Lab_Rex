...
/* PRIVATE ROUTINES */
void 		mgs_make_trials	(_PRtask);
_PRtrial mgs_get_trial	(_PRtask, int);
void 		mgs_set_trial	(_PRtrial);
...
struct _TAtask_struct gl_mgs_task[] = {
...
/* PUBLIC ROUTINE: mgs_make_trials
**
*/
void mgs_make_trials(_PRtask task)
{
	...
	*/
	if (TPMV(task, "asymRew"))	{
		...
		rewcont[1] = 1;
		ang_n = 2;
		angs  = toy_seriesi(ang_n, TPMV(task, "Angle_o"), 180);
	} else {
		rewcont_n = 1;
		rewcont = SAFE_ZALLOC(long, rewcont_n);
		rewcont[0] = 2;
		ang_n = toy_boundi(TPMV(task, "Angle_n"), 1, 1000);
		angs  = toy_seriesi(ang_n, TPMV(task, "Angle_o"), TPMV(task, "Angle_step"));
	}
	
		/* 
		** Make trial set for angle-reward contingency pairs
		*/
		tu_make_trials2D(task, "rewcont", rewcont_n, rewcont, 1.0, 	"angle", ang_n, angs, 1.0);
...
/* PUBLIC ROUTINE: mgs_get_trial
**
** Returns:
**		_PRtrial if trials remain
**		NULL otherwise
*/
_PRtrial mgs_get_trial(_PRtask task, int reset_flag)	
{
...
printf("MGS trial rewcont = %d angle = %d Ecc = %d \n", PL_L2PV(trial->list, 0), PL_L2PV(trial->list, 1), TPMV(trial->task, "ecc")  );
		PL_CSET( PL_L2PWS(tgm, 5, 1), PL_L2PV(trial->list, 1) );
