...
static int prevTargBest = -1; /* Need prevTargBest2 to differentiate from prob reward task */
static int runLength = 0; /* Need runLength2 to differentiate from prob reward task */
...
_PRtrial explorer_get_trial(_PRtask task, int reset_flag)
{
	...
	**		block_count2: the number of trials that
	**			have thus far been used in the current block
	**		block_index2: the index of the current block (i.e., the
	**			actual index in the 2D trial array in the task struct)
	**		block_menu_index2: the index of the relevant menu, which 
...
{ 
	...
	_PLproperty prop;
	valtype *xs, *ys;
	int target_distance  = TIMV(task, "target_distance"); 
	int i, *is;
	...
	*/
	if(PL_L2PV(dxTarget,1) != PL_L2PW(dxTarget,1) ||
		...
		for (i=0;i<ud->num_targets;i++) {
			is[i]     = i+2; /* assume fp/choice feedback are first 2 */
			xs[is[i]] = TOY_RT_TO_XF(0, target_distance/10., ud->angles[i]);
			ys[is[i]] = TOY_RT_TO_YF(0, target_distance/10., ud->angles[i]);
		}

		/* now set actual value from the working copy */
		dx_setw_by_nameIV(0, DX_TARGET, ud->num_targets, is, DX_X, DX_Y, NULL);
	...
	/* set choice target properties */
	dx_set_by_nameIV(0, DX_TARGET, 1, NULL,
		DX_DIAM, TIMV(task, "choice_diam"), 
		DX_CLUT, TIMV(task, "choice_clut"),
		NULL); 
